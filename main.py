...
class BlacklistPlugin(Star):
    def __init__(self, context: Context, config: AstrBotConfig):
        ...
        # 从配置加载默认拉黑时长
        self.default_blacklist_duration = self.config.get("default_blacklist_duration", 5)
    ...
    async def check_blacklist_before_llm(self, event: AstrMessageEvent, req: ProviderRequest):
        """拦截黑名单用户的请求（首次触发时获取Bot ID）"""
        # 首次处理请求时，通过事件获取Bot ID并初始化管理员
        ...
        # 拦截黑名单用户（未到解禁时间）
        if user_id in self.temporary_blacklist and time.time() < self.temporary_blacklist[user_id]:
            event.stop_event()
    ...
    async def handle_blacklist_request(self, event: AstrMessageEvent, duration_minutes: int = None):
        """处理拉黑请求（通过事件获取Bot ID）"""
        # 获取Bot ID（确保已初始化）
        ...
        if sender_id in self.administrators:
            ...
            await self._handle_normal_user_blacklist(sender_id, target_id, duration_minutes)

    async def auto_blacklist_by_bot(self, event: AstrMessageEvent, duration_minutes: int = None):
        """Bot自动拉黑违规用户（需传入事件对象）"""
        # 获取Bot ID
        self._get_bot_id(event)
    ...
    async def _handle_normal_user_blacklist(self, sender_id, target_id, duration):
        ...
        if target_id in self.administrators:
            ...
            logger.info(f"用户 {sender_id} 尝试拉黑管理员，已被反拉黑 {actual_duration} 分钟")
        # 仅允许拉黑自己
